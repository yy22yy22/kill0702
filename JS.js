...
function generateDMSetupInfo() {
    ...
    setupHtml += `<div style="background-color: #fff; padding: 10px; border-radius: 5px; margin-bottom: 15px; border: 1px dashed #ccc; text-align: left;">
        <strong>📊 战力探测：</strong> 相声清醒强神 <b>${H}</b> vs 搞笑强力 <b>${W}</b><br>
        <strong>⚖️ 局势判定：</strong> ${balanceMessage}
    </div>`;

    // ------------------------------------------
    // 2. 选取信息靶子 (必须是被视为搞笑阵营的人)
    // ------------------------------------------
    ...
    if (gaoFeng) {
        if (gaoFeng.isConfused) {
            setupHtml += `<li><b>【高峰】<span style="color: purple;">(混乱)</span>：</b>假信息：无论他选谁，全部告知<b>错误</b>的角色名。</li>`;
        } else {
            setupHtml += `<li><b>【高峰】(清醒)：</b>真信息：开局等待他选择。至少告知一人<b>正确</b>的角色。</li>`;
        }
    }
